...
#define PBD_SCALE 4//6//6//6//5//6//6//4//8//32//16//8//8//16//32//6//32//16//4	
		...
		float TriangleArea3D(float p1[3], float p2[3], float p3[3]);

		void InitSkinSurface(int v1, int v2, int v3);
		void DetectAndInitSkinSurface();
		void InitEdge();
		void InitVertices();
		void Update();
		void velocityUpdate();
		void InitParallelizableGroups();
		void Projection();
		...
		int PBDGPUMemoryAlloc(int E_SIZE, int CG_SIZE, int T_SIZE, int T_CG_SIZE, int S_SIZE, int S_TG_SIZE);
		int GPUtoCP();
		int CPtoGPU();
		void PBDGPUMemoryFree();

		int cuda_SolveDistanceConstraint(int Dgroup[], int CG_SIZE);
		int cuda_SolveVolumeConstraint(int Vgroup[], int T_CG_SIZE);
		int cuda_SolveDihedralConstraint(int Vgroup[], int T_CG_SIZE);
		int cuda_SolveSkinTensionConstraint(int Sgroup[], int SG_SIZE);

		int saved_option = -1; // Variable to store the saved option index

		void saveOptionToCuda(int option);
		void RenderUI();
		...
		void initPBD();
		void openPBDWindow(int option);
		int cudamain(float eye[3]);
		cudaArray* mallocArr(vec4* pvol, cudaExtent volumeExtend);
		cudaArray* mallocArr2(float4* pvol, cudaExtent volumeExtend);
		cudaError_t InitData();
		bool initTexture(cudaTextureObject_t* pTex, cudaArray* pDevVol);
		bool initTexture2(cudaTextureObject_t* pTex, cudaArray* pDevVol);
		//void brightenImageKernel(float* input, unsigned char* output);
		cudaError_t addWithCuda(unsigned char* out);
		cudaError_t addWithCuda2(unsigned char* out);
		cudaError_t addWithCuda3(unsigned char* out);
		...
		float m_dynamic_res_target_fps = 1.0f;
