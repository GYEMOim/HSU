...
namespace ngp {
	...
	void Testbed::imgui() {
		...
		if (ImGui::CollapsingHeader("Training", m_training_data_available ? ImGuiTreeNodeFlags_DefaultOpen : 0)) {
			...
			ImGui::PlotLines("loss graph", m_loss_graph.data(), std::min(m_loss_graph_samples, m_loss_graph.size()), (m_loss_graph_samples < m_loss_graph.size()) ? 0 : (m_loss_graph_samples % m_loss_graph.size()), 0, FLT_MAX, FLT_MAX, ImVec2(0, 400.f));
		...
		/*if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.3f);
			if (m_dynamic_res) {
				ImGui::SliderFloat("Target FPS", &m_dynamic_res_target_fps, 1.0f, 2.0f, "%.01f", ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
			}
			else {
		...
		if (m_testbed_mode == ETestbedMode::Nerf || m_testbed_mode == ETestbedMode::Sdf) {
			if (ImGui::CollapsingHeader("Open Rendering Option")) {
	...
	void Testbed::train_and_render(bool skip_rendering) {
		...
		if (render_flag) {
			...
			Testbed::openPBDWindow(selected_option);
			render_flag = false;
		...
		//
		//		//!
		//		//!m_render_window || !m_render || skip_rendering) {
		//		if (false){
		//			return;
		//		}
		//
	...
	void Testbed::init_window(int resw, int resh, bool hidden, bool second_window) {
		...
		//우리꺼 윈도우창
		//secondWindow = glfwCreateWindow(640, 480, "Second Window", NULL, NULL); // 마지막 인자는 공유 컨텍스트
		//if (secondWindow == NULL) {
		//	throw std::runtime_error{ "Second GLFW window could not be created." };
		//}
